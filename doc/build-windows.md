...
Below are some notes on how to build Rubas Core for Windows.

The options known to work for building Rubas Core on Windows are:
...
Note that for WSL the Rubas Core source path MUST be somewhere in the default mount file system, for
example /usr/src/rubas, AND not under /mnt/d/. If this is not the case the dependency autoconf scripts will fail.
...
way. This will install to `c:\workspace\rubas`, for example:

    make install DESTDIR=/mnt/c/workspace/rubas
