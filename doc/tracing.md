# User-space, Statically Defined Tracing (USDT) for Rubas Core

Rubas Core includes statically defined tracepoints to allow for more
...
**Note**: Rubas Core uses temporary clones of the _main_ UTXO cache
...
## Adding tracepoints to Rubas Core
...
Multiple tools can list the available tracepoints in a `rubasd` binary with
USDT support.

### GDB - GNU Project Debugger

To list probes in Rubas Core, use `info probes` in `gdb`:
...
stap net        inbound_message  0x000000000014419e /src/rubasd
stap net        outbound_message 0x0000000000107c05 /src/rubasd
stap validation block_connected  0x00000000002fb10c /src/rubasd
…
```

### With `readelf`

The `readelf` tool can be used to display the USDT tracepoints in Rubas Core.
Look for the notes with the description `NT_STAPSDT`.

```
$ readelf -n ./src/rubasd | grep NT_STAPSDT -A 4 -B 2
