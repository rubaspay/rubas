...
---------------------------------------------------------
The MN_RR hard fork, first included in Rubas Core v20, will be activated after v21 is adopted by masternodes. This
hard fork enables the major feature included in this release: Masternode Reward Location Reallocation. The activation
will also initiate the launch of the Rubas Platform Genesis Chain.

As the MN_RR hard fork is an [Enhanced Hard Fork](https://github.com/rubaspay/dips/blob/master/dip-0023.md),
...
The Rubas Core repository (`github.com/rubaspay/rubas`) will now use `develop` as the default branch. New clones
of the repository will no longer default to the `master` branch. If you want to continue using a stable version of
Rubas Core, you should manually checkout the `master` branch.
...
can be adjusted using the new `-maxapsfee` configuration option. (rubas#5930)

### Experimental Descriptor Wallets

Please note that Descriptor Wallets are still experimental and not all expected functionality
is available. Additionally there may be some bugs and current functions may change in the future.
Bugs and missing functionality can be reported to the [issue tracker](https://github.com/rubaspay/rubas/issues).
...
Users who already have a Rubas Core wallet can continue to use it as they did before without
any change in behavior. Newly created Legacy Wallets (which is the default type of wallet) will
behave as they did in previous versions of Rubas Core.
...
- The `-banscore` configuration option, which modified the default threshold for
  ...
  changes in this release to the handling of misbehaving peers. (rubas#5511)
...
- A new `listdescriptors` RPC is available to inspect the contents of descriptor-enabled wallets.
  ...
  For ranged descriptors, it also returns the range boundaries and the next index to generate addresses from. (rubas#5911)
...
- Backwards compatibility has been dropped for two `getaddressinfo` RPC
  ...
  20.1, the `labels` field returns a JSON array of label names. (rubas#5823)
- `getnetworkinfo` now returns fields `connections_in`, `connections_out`,
  ...
  `outboundmnconnections` and `mnconnections` are removed (rubas#5823)
- `getpeerinfo` no longer returns the `banscore` field unless the configuration
  option `-deprecatedrpc=banscore` is used. The `banscore` field will be fully
  removed in the next major release. (rubas#5511)
...
Rubas Core's composite commands such as `quorum list` or `bls generate` now are compatible with a whitelist feature.
...
silently overwriting earlier parameter values with later ones. (rubas#6050)
...
- A rubasd node will no longer broadcast addresses to inbound peers by default.
...
rubas-tx Changes
---------------
- When creating a hex-encoded Rubas transaction using the `rubas-tx` utility
...
- A new `-rpcwaittimeout` argument to `rubas-cli` sets the timeout
  in seconds to use with `-rpcwait`. If the timeout expires,
  `rubas-cli` will report a failure. (rubas#5923)
- The `connections` field of `rubas-cli -getinfo` is expanded to return a JSON
  ...
  returned a single integer value for the total number of peer connections. (rubas#5823)
...
- CLI `-addrinfo` now returns a single field for the number of `onion` addresses
  ...
  for Tor V2 addresses was removed from Rubas Core in 18.0.

rubas-wallet changes
-------------------
This release introduces several improvements and new features to the `rubas-wallet` tool, making it more versatile and
user-friendly for managing Rubas wallets.

- Wallets created with the `rubas-wallet` tool will now utilize the `FEATURE_LATEST` version of wallet which is the HD
...
[set-of-changes]: https://github.com/rubaspay/rubas/compare/v20.1.1...rubaspay:v21.0.0
