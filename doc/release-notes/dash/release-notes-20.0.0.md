...
Currently, 10% of block rewards are set aside for the Rubas DAO treasury which
...
The asset lock process provides a mechanism for locking and unlocking Rubas in a
credit pool. This credit pool mirrors the available credit supply in Rubas
Platform. When a certain amount of Rubas is locked, a proportional number of
credits are issued to the identity specified by the transaction issuer. The
identity then uses the credits to pay fees for operations done on Rubas
Platform. Masternode identities receive the fees and can convert them back to
Rubas by withdrawing them from Rubas Platform.
...
This feature has been disabled by default since Rubas Core version 19.0.0.
Nodes on the network can not generally be trusted to send valid ("reject")
messages, so this should only ever be used when connected to a trusted node.

Since Rubas Core version 20.0.0 there are extra changes:
...
- It is possible to run Rubas Core as an I2P service and connect to such
...
- The `fundrawtransaction`, `sendmany`, `sendtoaddress`, and
  ...
  fee estimation methods, `RUBAS/kB` and `duff/B`. The target is the fee
...
- `testmempoolaccept` and `sendrawtransaction` no longer return the P2P
  ...
  Section [_Removal of reject network messages from Rubas Core (BIP61)_](#removal-of-reject-network-messages-from-rubas-core-bip61) above for
...
- The `rubas-cli -getinfo` command now displays the wallet name and balance for
...
python test framework and `rubas-cli` tool both take advantage of this, so
for example:

```sh
rubas-cli -named createwallet wallet_name=mywallet load_on_startup=1
```

Can now be shortened to:

```sh
rubas-cli -named createwallet mywallet load_on_startup=1
```
...
with Rubas’s product needs, such as SegWit and RBF, are excluded from our
...
[DIP-0023]: https://github.com/rubaspay/dips/blob/master/dip-0023.md
[DIP-0029]: https://github.com/rubaspay/dips/blob/master/dip-0029.md

[i2p]: https://github.com/rubaspay/rubas/blob/v20.x/doc/i2p.md
[guix-readme]: https://github.com/rubaspay/rubas/blob/v20.x/contrib/guix/README.md
[json-rpc]: https://github.com/rubaspay/rubas/blob/v20.x/doc/JSON-RPC-interface.md#parameter-passing
[set-of-changes]: https://github.com/rubaspay/rubas/compare/v19.3.0...rubaspay:v20.0.0
