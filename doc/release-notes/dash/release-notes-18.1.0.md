...
Rubas Core imperfectly follows semantic versioning. Breaking changes should be
...
Support for the BIP70 Payment Protocol has been dropped from Rubas Qt.
...
The systemd init file (`contrib/init/rubasd.service`) has been changed to use
`/var/lib/rubasd` as the data directory instead of `~rubas/.rubas`. This
change makes Rubas Core more consistent with other services, and makes the
...
`datadir` option in `/etc/rubas/rubas.conf` will have no effect. This is
because the command line arguments specified in the init files take precedence
over the options specified in `/etc/rubas/rubas.conf`.
...
See the [release notes from v18.0.1](https://github.com/rubaspay/rubas/blob/v18.0.1/doc/release-notes.md) for a full description of the changes from the
...
the deprecation of wallet accounts in RubasCore v0.17 and introduction of PSBT
format.
...
- The `testnet` field in `rubas-cli -getinfo` has been renamed to `chain` and now returns the current network name as defined in BIP70 (main, test, regtest).
...
This release introduces many hundreds updates from Bitcoin v0.18/v0.19/v0.20/v0.21/v22. Bitcoin changes that do not align with Rubas’s product needs, such as SegWit and RBF, are excluded from our backporting. For additional detail on what’s included in Bitcoin, please refer to their release notes.

v18.1.0 Change log
==================

See detailed [set of changes](https://github.com/rubaspay/rubas/compare/v18.0.2...rubaspay:v18.1.0).
