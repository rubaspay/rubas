...
In this release, we are taking a significant step towards enhancing the Rubas wallet's usability by enabling Hierarchical
Deterministic (HD) wallets by default. This change aligns the behavior of `rubasd` and `rubas-qt` with the previously
optional `-usehd=1` flag, making HD wallets the standard for all users.

While HD wallets are now enabled by default to improve user experience, Rubas Core still allows for the creation of
...
Modified the Dockerfile to exclude the `rubas-qt` graphical wallet interface from the set of binaries copied to
`/usr/local/bin` during the Docker image build process. This change streamlines the Docker image, making it more
suitable for server environments and headless applications, where the graphical user interface of `rubas-qt` is not
...
the v18.1 release.
- The `startupnotify` option is used to specify a command to execute when Rubas Core has finished with its startup
sequence.
...
- New Discreet mode for hiding balances in Rubas-Qt
- Added a "Close all wallet" menu item for Rubas-Qt
...
[set-of-changes]: https://github.com/rubaspay/rubas/compare/v20.0.4...rubaspay:v20.1.0
