# CJDNS support in Rubas Core

It is possible to run Rubas Core over CJDNS, an encrypted IPv6 network that
...
done outside of Rubas Core, similarly to a VPN (either in the host/OS or on
the network router).
...
redundancy and robustness for both the Rubas network and individual nodes.
...
## Run Rubas Core with CJDNS

Once you are connected to the CJDNS network, the following Rubas Core
...
When enabled, this option tells Rubas Core that it is running in an
...
IPv6 local network. This helps Rubas Core perform better address management:
...
CJDNS support was added to Rubas Core in version 21.0 and there may be fewer
CJDNS peers than Tor or IP ones. You can use `rubas-cli -addrinfo` to see the
...
[doc/tor.md](https://github.com/rubaspay/rubas/blob/master/doc/tor.md) for
details.

## CJDNS-related information in Rubas Core

There are several ways to see your CJDNS address in Rubas Core:
- in the "Local addresses" output of CLI `-netinfo`
- in the "localaddresses" output of RPC `getnetworkinfo`

To see which CJDNS peers your node is connected to, use `rubas-cli -netinfo 4`
or the `getpeerinfo` RPC (i.e. `rubas-cli getpeerinfo`).
