# Fuzzing Rubas Core using libFuzzer

## Quickstart guide

To quickly get started fuzzing Rubas Core using [libFuzzer](https://llvm.org/docs/LibFuzzer.html):

```sh
$ git clone https://github.com/rubaspay/rubas
$ cd rubas/
$ ./autogen.sh
$ CC=clang CXX=clang++ ./configure --enable-fuzz --with-sanitizers=address,fuzzer,undefined
# macOS users: If you have problem with this step then make sure to read "macOS hints for
# libFuzzer" on https://github.com/rubaspay/rubas/blob/develop/doc/fuzzing.md#macos-hints-for-libfuzzer
$ make
...
[`process_message`](https://github.com/rubaspay/rubas/blob/develop/src/test/fuzz/process_message.cpp) is a fuzzing harness for the [`ProcessMessage(...)` function (`net_processing`)](https://github.com/rubaspay/rubas/blob/develop/src/net_processing.cpp). The available fuzzing harnesses are found in [`src/test/fuzz/`](https://github.com/rubaspay/rubas/tree/develop/src/test/fuzz).
...
It is possible to specify `rubasd` arguments to the `fuzz` executable.
Depending on the test, they may be ignored or consumed and alter the behavior
of the test. Just make sure to use double-rubas to distinguish them from the
...
Every single pull request submitted against the Rubas Core repo is automatically tested against all inputs in the [`bitcoin-core/qa-assets`](https://github.com/bitcoin-core/qa-assets) repo. Contributing new coverage increasing inputs is an easy way to help make Rubas Core more robust.
...
with certain assembly code from Rubas Core's code. See [developer notes on sanitizers](https://github.com/rubaspay/rubas/blob/develop/doc/developer-notes.md#sanitizers)
...
# Fuzzing Rubas Core using afl++

## Quickstart guide

To quickly get started fuzzing Rubas Core using [afl++](https://github.com/AFLplusplus/AFLplusplus):
...
# Fuzzing Rubas Core using Honggfuzz

## Quickstart guide

To quickly get started fuzzing Rubas Core using [Honggfuzz](https://github.com/google/honggfuzz):
...
## Fuzzing the Rubas Core P2P layer using Honggfuzz NetDriver

Honggfuzz NetDriver allows for very easy fuzzing of TCP servers such as Rubas
Core without having to write any custom fuzzing harness. The `rubasd` server
...
$ honggfuzz/honggfuzz --exit_upon_crash --quiet --timeout 4 -n 1 -Q \
      -E HFND_TCP_PORT=18444 -f inputs/ -- \
          src/rubasd -regtest -discover=0 -dns=0 -dnsseed=0 -listenonion=0 \
...
program, which includes a rubasboard of [publicly disclosed vulnerabilities](https://bugs.chromium.org/p/oss-fuzz/issues/list).
