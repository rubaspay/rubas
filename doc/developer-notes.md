...
- [Development guidelines](#development-guidelines)
    - [General Rubas Core](#general-rubas-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
Rubas Core uses [Doxygen](https://www.doxygen.nl/) to generate its official documentation.
...
configuration options and toggled while rubasd is running with the `logging`
RPC.  For instance, launching rubasd with `-debug` or `-debug=1` will turn on
...
Rubas Core is a multi-threaded application, and deadlocks or other
...
i.e. `CPPFLAGS="-DDEBUG_LOCKCONTENTION"`, then build and run rubasd.

You can then use the `-debug=lock` configuration option at rubasd startup or
`rubas-cli logging '["lock"]'` at runtime to turn on lock contention logging.
It can be toggled off again with `rubas-cli logging [] '["lock"]'`.
...
([`valgrind.supp`](https://github.com/rubaspay/rubas/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_rubas
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_rubas --log_level=test_suite
$ valgrind -v --leak-check=full src/rubasd -printtoconsole
...
# A coverage report will now be accessible at `./test_rubas.coverage/index.html`.
```
...
To profile a running rubasd process for 60 seconds, you could use an
invocation of `perf record` like this:

```sh
$ perf record \
    -g --call-graph dwarf --per-thread -F 140 \
    -p `pgrep rubasd` -- sleep 60
```
...
Rubas Core can be compiled with various "sanitizers" enabled, which add
...
- [Main thread (`rubasd`)](https://doxygen.bitcoincore.org/bitcoind_8cpp.html#a0ddf1224851353fc92bfbff6f499fa97)
...
However, in open source software such as Rubas Core, where everyone uses
...
pay attention to for reviewers of Rubas Core code.
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers.
  ...
  - *Rationale*: Rubas Core uses tinyformat, which is type safe. Leave them out to avoid confusion.
...
be sent upstream, but bugfixes may also be prudent to PR against Rubas Core so that they can be integrated
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `rubas-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `rubas-cli` and the GUI debug console use this table to determine how to
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  ...
  - *Rationale*: In previous versions of Rubas Core, the wallet was always
...
- Use *invalid* Rubas addresses (e.g. in the constant array `EXAMPLE_ADDRESS`) for
