...
* [ ] Update translations, see [translation_process.md](https://github.com/rubaspay/rubas/blob/master/doc/translation_process.md#synchronising-translations).
* [ ] Update manpages (after rebuilding the binaries), see [gen-manpages.py](https://github.com/bitcoin/bitcoin/blob/master/contrib/devtools/README.md#gen-manpagespy).

Before every minor and major release:

* [ ] Update [bips.md](bips.md) to account for changes since the last release.
* [ ] Update DIPs with any changes introduced by this release (see [this pull request](https://github.com/rubaspay/dips/pull/142) for an example)
...
* [ ] Update hardcoded [seeds](/contrib/seeds/README.md), see [this pull request](https://github.com/rubaspay/rubas/pull/5914) for an example.
* [ ] Update [`src/chainparams.cpp`](/src/chainparams.cpp) `m_assumed_blockchain_size` and `m_assumed_chain_state_size` with the current size plus some overhead (see [this](#how-to-calculate-assumed-blockchain-and-chain-state-size) for information on how to calculate them).
* [ ] Update [`src/chainparams.cpp`](/src/chainparams.cpp) `chainTxData` with statistics about the transaction count and rate. Use the output of the `getchaintxstats` RPC, see
  [this pull request](https://github.com/rubaspay/rubas/pull/5692) for an example. Reviewers can verify the results by running `getchaintxstats <window_block_count> <window_last_block_hash>` with the `window_block_count` and `window_last_block_hash` from your output.
...
git clone https://github.com/rubaspay/guix.sigs.git
git clone https://github.com/rubaspay/rubas-detached-sigs.git
git clone https://github.com/rubaspay/rubas.git
```

### Rubas Core maintainers/release engineers, suggestion for writing release notes
...
Checkout the Rubas Core version you'd like to build:
...
_Note: this step can be skipped if [our CI](https://github.com/rubaspay/rubas/blob/master/ci/test/00_setup_env.sh#L64) still uses bitcoin's SDK package (see SDK_URL)_
...
Add other builders keys to your gpg keyring, and/or refresh keys: See `../rubas/contrib/builder-keys/README.md`.
...
* Transfer `rubascore-osx-unsigned.tar.gz` to macOS for signing
* Extract and sign:

    ```sh
    tar xf rubascore-osx-unsigned.tar.gz
...
* Extract and sign:
    ...
    ```sh
    tar xf rubascore-win-unsigned.tar.gz
...
- Detached signatures will then be committed to the [rubas-detached-sigs](https://github.com/rubaspay/rubas-detached-sigs) repository, which can be combined with the unsigned apps to create signed binaries.
...
* [ ] Upload zips and installers, as well as `SHA256SUMS.asc` from last step, to GitHub as GitHub draft release.
    1. The contents of each `./rubas/guix-build-${VERSION}/output/${HOST}/` directory, except for
       `*-debug*` files.
       ...
       as save storage space *do not upload these to the rubas.org server*.

       ```sh
       find guix-build-${VERSION}/output/ -maxdepth 2 -type f -not -name "SHA256SUMS.part" -and -not -name "*debug*" -exec scp {} user@rubas.org:/var/www/bin/rubas-core-${VERSION} \;
       ```
...
* [ ] Update the rubas.org download links
* [ ] Ensure that docker hub images are up to date

### Announce the release:
* [ ] Release on Rubas forum: https://www.rubas.org/forum/topic/official-announcements.54/ (necessary so we have a permalink to use on twitter, reddit, etc.)
...
### After the release:
* [ ] Submit patches to BTCPay to ensure they use latest / compatible version see https://github.com/rubaspay/rubas/issues/4211#issuecomment-966608207
* [ ] Update Core and User docs (docs.rubas.org)
* [ ] Test Docker build runs without error in Rubasmate
...
```sh
xcrun notarytool submit rubascore-{version}-{x86_64, arm64}-apple-darwin.dmg --keychain-profile "AC_PASSWORD" --wait
```
...
After successfully notarizing the .dmg file, extract `Rubas-Qt.app` from the .dmg.
To verify that the notarization process was successful, run the following command:

```sh
spctl -a -vv -t install Rubas-Qt.app
```

Replace `Rubas-Qt.app` with the path to your .app file. This command checks whether your .app file passes Gatekeeper’s
