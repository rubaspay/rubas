...
On unix systems, the `--enable-multiprocess` build option can be passed to `./configure` to build new `rubas-node`, `rubas-wallet`, and `rubas-gui` executables alongside existing `rubasd` and `rubas-qt` executables.

`rubas-node` is a drop-in replacement for `rubasd`, and `rubas-gui` is a drop-in replacement for `rubas-qt`, and there are no differences in use or external behavior between the new and old executables. But internally (after backporting [bitcoin#10102](https://github.com/bitcoin/bitcoin/pull/10102)), `rubas-gui` will spawn a `rubas-node` process to run P2P and RPC code, communicating with it across a socket pair, and `rubas-node` will spawn `rubas-wallet` to run wallet code, also communicating over a socket pair. This will let node, wallet, and GUI code run in separate address spaces for better isolation, and allow future improvements like being able to start and stop components independently on different machines and environments.

## Next steps

Specific next steps after backporting [bitcoin#10102](https://github.com/bitcoin/bitcoin/pull/10102) will be:

- [ ] Adding `-ipcbind` and `-ipcconnect` options to `rubas-node`, `rubas-wallet`, and `rubas-gui` executables so they can listen and connect to TCP ports and unix socket paths. This will allow separate processes to be started and stopped any time and connect to each other.
- [ ] Adding `-server` and `-rpcbind` options to the `rubas-wallet` executable so wallet processes can handle RPC requests directly without going through the node.
...
```
cd <RUBAS_SOURCE_DIRECTORY>
make -C depends NO_QT=1 MULTIPROCESS=1
CONFIG_SITE=$PWD/depends/x86_64-pc-linux-gnu/share/config.site ./configure
make
src/rubas-node -regtest -printtoconsole -debug=ipc
RUBASD=rubas-node test/functional/test_runner.py
```
