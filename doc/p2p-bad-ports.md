When Rubas Core automatically opens outgoing P2P connections, it chooses
...
A malicious actor may gossip an address:port where no Rubas node is listening,
or one where a service is listening that is not related to the Rubas network.
As a result, this service may occasionally get connection attempts from Rubas
nodes.

"Bad" ports are ones used by services which are usually not open to the public
and usually require authentication. A connection attempt (by Rubas Core,
trying to connect because it thinks there is a Rubas node on that
...
They are also considered "bad" ports as they require clients to either run Rubas
...
Below is a list of "bad" ports which Rubas Core avoids when choosing a peer to
