...
"""Test objects for interacting with a rubasd node over the p2p protocol.

The P2PInterface objects interact with the rubasd nodes under test using the
...
class P2PConnection(asyncio.Protocol):
    ...
    def peer_connect(self, dstaddr, dstport, *, net, timeout_factor, supports_v2_p2p, uacomment=None):
        ...
        logger.debug('Connecting to Rubas Node: %s:%d' % (self.dstaddr, self.dstport))
    ...
    def peer_accept_connection(self, connect_id, connect_cb=lambda: None, *, net, timeout_factor, supports_v2_p2p, reconnect, uacomment=None):
        ...
        logger.debug('Listening for Rubas Node with id: {}'.format(connect_id))
...
class P2PInterface(P2PConnection):
    """A high-level P2P interface class for communicating with a Rubas node.
