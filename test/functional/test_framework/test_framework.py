...
TMPDIR_PREFIX = "rubas_func_test_"
...
class BitcoinTestFramework(metaclass=BitcoinTestMetaClass):
    ...
    def parse_args(self):
        ...
        parser.add_argument("--nocleanup", dest="nocleanup", default=False, action="store_true",
                            help="Leave rubasds and test.* datadir on exit or error")
        parser.add_argument("--noshutdown", dest="noshutdown", default=False, action="store_true",
                            help="Don't stop rubasds after the test execution")
        ...
        parser.add_argument("--usecli", dest="usecli", default=False, action="store_true",
                            help="use rubas-cli instead of RPC for all commands")
        parser.add_argument("--rubasd-arg", dest="rubasd_extra_args", default=[], action="append",
                            help="Pass extra args to all rubasd instances")
    ...
    def setup(self):
        ...
        fname_bitcoind = os.path.join(
            ...
            "src",
            "rubasd" + config["environment"]["EXEEXT"],
        )
        fname_bitcoincli = os.path.join(
            config["environment"]["BUILDDIR"],
            "src",
            "rubas-cli" + config["environment"]["EXEEXT"],
        )
        self.options.bitcoind = os.getenv("BITCOIND", default=fname_bitcoind)
        self.options.bitcoincli = os.getenv("BITCOINCLI", default=fname_bitcoincli)

        self.extra_args_from_options = self.options.rubasd_extra_args
    ...
    def shutdown(self):
        ...
        if not self.options.noshutdown:
            ...
            self.log.info("Note: rubasds were not stopped and may still be running")
    ...
    def setup_nodes(self):
        ...
        """If this method is updated - backport changes to  RubasTestFramework.setup_nodes"""
    ...
    def add_nodes(self, num_nodes: int, extra_args=None, *, rpchost=None, binary=None, binary_cli=None, versions=None):
        ...
        if binary is None:
            binary = [get_bin_from_version(v, 'rubasd', self.options.bitcoind) for v in versions]
        if binary_cli is None:
            binary_cli = [get_bin_from_version(v, 'rubas-cli', self.options.bitcoincli) for v in versions]
    ...
    def dynamically_initialize_datadir(self, node_p2p_port, node_rpc_port):
        ...
        with open(os.path.join(new_data_dir, "rubas.conf"), 'w', encoding='utf8') as f:
    ...
    def _start_logging(self):
        ...
        ch.setLevel(ll)
        # Format logs the same as rubasd's debug.log with microprecision (so log files can be concatenated and sorted)
    ...
    def _initialize_chain(self):
        ...
        for i in range(self.num_nodes):
            ...
            initialize_datadir(self.options.tmpdir, i, self.chain)  # Overwrite port/rpcport in rubas.conf
    ...
    def skip_if_no_bitcoind_tracepoints(self):
        """Skip the running test if rubasd has not been compiled with USDT tracepoint support."""
        if not self.is_usdt_compiled():
            raise SkipTest("rubasd has not been built with USDT tracepoints enabled.")
    ...
    def skip_if_no_bitcoind_zmq(self):
        """Skip the running test if rubasd has not been compiled with zmq support."""
        if not self.is_zmq_compiled():
            raise SkipTest("rubasd has not been built with zmq enabled.")
    ...
    def skip_if_no_wallet_tool(self):
        """Skip the running test if rubas-wallet has not been compiled."""
        if not self.is_wallet_tool_compiled():
            raise SkipTest("rubas-wallet has not been compiled")

    def skip_if_no_cli(self):
        """Skip the running test if rubas-cli has not been compiled."""
        if not self.is_cli_compiled():
            raise SkipTest("rubas-cli has not been compiled.")
    ...
    def is_cli_compiled(self):
        """Checks whether rubas-cli was compiled."""
    ...
    def is_wallet_tool_compiled(self):
        """Checks whether rubas-wallet was compiled."""
...
class RubasTestFramework(BitcoinTestFramework):
    def set_test_params(self):
    ...
    def set_rubas_test_params(self, num_nodes, masterodes_count, extra_args=None, evo_count=0):
        ...
        # This is nRequestTimeout in rubas-q-recovery thread
    ...
    def set_rubas_llmq_test_params(self, llmq_size, llmq_threshold):
