...
class ConfArgsTest(BitcoinTestFramework):
    ...
    def test_config_file_parser(self):
        ...
        with open(os.path.join(self.nodes[0].datadir, 'rubas.conf'), 'a', encoding='utf-8') as conf:
            conf.write(f'includeconf={inc_conf_file_path}\n')

        self.nodes[0].assert_start_raises_init_error(
            expected_msg='Error: Error parsing command line arguments: Invalid parameter -rubas_cli=1',
            extra_args=['-rubas_cli=1'],
        )
        with open(inc_conf_file_path, 'w', encoding='utf-8') as conf:
            conf.write('rubas_conf=1\n')
        with self.nodes[0].assert_debug_log(expected_msgs=['Ignoring unknown configuration value rubas_conf']):
            self.start_node(0)
        self.stop_node(0)

        with open(inc_conf_file_path, 'w', encoding='utf-8') as conf:
            conf.write('-rubas=1\n')
        self.nodes[0].assert_start_raises_init_error(expected_msg='Error: Error reading configuration file: parse error on line 1: -rubas=1, options in configuration file must be specified without leading -')
        ...
        main_conf_file_path = os.path.join(self.options.tmpdir, 'node0', 'rubas_main.conf')
    ...
    def run_test(self):
        ...
        conf_file = os.path.join(default_data_dir, "rubas.conf")
        ...
        none_existent_conf_file = os.path.join(default_data_dir, "none_existent_rubas.conf")
