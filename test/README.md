This directory contains integration tests that test rubasd and its
...
rubasd and rubas-qt by interacting with them through the RPC and P2P
interfaces.
- [util](/test/util) which tests the utilities (rubas-tx, ...).
...
Before tests can be run locally, Rubas Core must be built.  See the [building instructions](/doc#building) for help.
...
Many Rubas specific tests require rubas_hash. To install it:

- Clone the repo `git clone https://github.com/rubaspay/rubas_hash`
- Install rubas_hash `cd rubas_hash && pip3 install -r requirements.txt .`
...
The P2P and RPC ports used by the rubasd nodes-under-test are chosen to make
conflicts with other processes unlikely. However, if there is another rubasd
process running on the system (perhaps from a previous test which hasn't successfully
killed all its rubasd nodes), then there may be a port conflict which will
cause the test to fail. It is recommended that you run the tests on a system
where no other rubasd processes are running.

On linux, the test framework will warn if there is another
rubasd process running when the tests are started.

If there are zombie rubasd processes after test failure, you can kill them
by running the following commands. **Note that these commands will kill all
rubasd processes running on the system, so should not be used if any non-test
rubasd processes are being run.**
...
sure rubasd processes are stopped as above):
...
- when run by [our CI (Continuous Integration)](/ci/README.md), no logs are output to the console. However, if a test
  fails, the `test_framework.log` and rubasd `debug.log`s will all be dumped
...
`test_framework.log` and rubasd `debug.log`s can be combined into a single
...
call methods that interact with the rubasd nodes-under-test.

If further introspection of the rubasd instances themselves becomes
...
```bash
cat /tmp/user/1000/testo9vsdjo3/node1/regtest/rubasd.pid
```

Then you can use the pid to start `gdb`:

```bash
gdb /home/example/rubasd <pid>
```
