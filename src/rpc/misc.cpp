...
static RPCHelpMan debug()
{
    return RPCHelpMan{"debug",
        ...
        " - \"rubas\" activates all Rubas-specific categories at once;\n"
        ...
        RPCExamples {
            HelpExampleCli("debug", "rubas")
    + HelpExampleRpc("debug", "rubas+net")
        },
...
static RPCHelpMan validateaddress()
{
    return RPCHelpMan{"validateaddress",
                ...
                {
                    {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "The Rubas address to validate"},
                },
                RPCResult{
                    RPCResult::Type::OBJ, "", "",
                    {
...
static RPCHelpMan echo(const std::string& name)
{
    return RPCHelpMan{name,
                ...
                "rubas-cli and the GUI. There is no server-side difference.",
                {
    ...
    return RPCHelpMan{
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue {
            ...
            if (interfaces::Ipc* ipc = Assert(EnsureAnyNodeContext(request.context).init)->ipc()) {
                ...
                auto init = ipc->spawnProcess("rubas-node");
                echo = init->makeEcho();
