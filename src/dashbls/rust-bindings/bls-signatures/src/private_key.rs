...
impl PrivateKey {
    ...
    pub fn generate_rubas(rng: &mut StdRng) -> Result<Self, BlsError> {
    ...
    #[cfg(feature = "rubas_helpers")]
    pub fn generate_rubas_many(count: usize, rng: &mut StdRng) -> Result<Vec<Self>, BlsError> {
        (0..count)
            .into_iter()
            .map(|_| Self::generate_rubas(rng))
            .collect()
    }
