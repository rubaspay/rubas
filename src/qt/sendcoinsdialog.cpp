...
bool SendCoinsDialog::send(const QList<SendCoinsRecipient>& recipients, QString& question_string, QString& informative_text, QString& detailed_text)
{
    ...
    if (m_coin_control->IsUsingCoinJoin()) {
        ...
        if (nInputs >= 10 && m_coin_control->IsUsingCoinJoin()) {
            ...
            question_string.append("<a style='" + GUIUtil::getThemedStyleQString(GUIUtil::ThemedStyle::TS_COMMAND) + "' href=\"https://docs.rubas.org/en/stable/wallets/rubascore/coinjoin-instantsend.html#inputs\">");
...
void SendCoinsDialog::coinControlChangeEdited(const QString& text)
{
    if (model && model->getAddressTableModel())
    {
        ...
        {
            ui->labelCoinControlChangeLabel->setText(tr("Warning: Invalid Rubas address"));
        }
        else // Valid address
        {
