 ...
 */
static const char* BITCOIN_PID_FILENAME = "rubasd.pid";
...
void SetupServerArgs(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-sporkaddr=<rubasaddress>", "Override spork address. Only useful for regtest and devnet. Using this on mainnet or testnet will ban you.", ArgsManager::ALLOW_ANY, OptionsCategory::DEBUG_TEST);
    ...
    argsman.AddArg("-deprecated-platform-user=<user>", "Set the username for the \"platform user\", a restricted user intended to be used by Rubas Platform, to the specified username.", ArgsManager::ALLOW_ANY, OptionsCategory::MASTERNODE);
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/rubaspay/rubas>";
...
{
    // Make sure only a single Rubas Core process is using the data directory.
...
bool AppInitMain(NodeContext& node, interfaces::BlockAndHeaderTipInfo* tip_info)
{
    ...
    if (args.IsArgSet("-datadir") && !args.GetPathArg("-datadir").is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the " /* Continued */
                  "current working directory '%s'. This is fragile, because if Rubas Core is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if Rubas Core is started while in a temporary directory.\n",
    ...
    // ********************************************************* Step 7d: Setup other Rubas services
    ...
    // ********************************************************* Step 10a: schedule Rubas-specific tasks
    ...
    const auto BadPortWarning = [](const char* prefix, uint16_t port) {
        return strprintf(_("%s request to listen on port %u. This port is considered \"bad\" and "
                           "thus it is unlikely that any Rubas Core peers connect to it. See "
