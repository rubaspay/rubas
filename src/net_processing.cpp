 ...
 *  We have 4 times smaller block times in Rubas, so we need to push 4 times more invs per 1MB. */
...
struct Peer {
     ...
     * (Rubas)    Enables the flag that allows GetTxRelay() to return m_tx_relay */
...
private:
     ...
     * (Rubas)    Always initialized but selectively available through GetTxRelay()
...
bool PeerManagerImpl::AlreadyHave(const CInv& inv)
{
    ...
    /*
        Rubas Related Inventory Messages
...
bool PeerManagerImpl::SendMessages(CNode* pto)
{
...

    {
        ...
        // RUBAS this code also handles non-TXs (Rubas specific messages)
