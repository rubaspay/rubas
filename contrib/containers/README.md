...
Currently two Docker containers exist, `ci` defines how Rubas's GitLab CI container is built and the `dev` builds on top of the `ci` to provide a containerized development environment that is as close as possible to CI for contributors! See also [Rubas on Docker Hub](https://hub.docker.com/u/rubaspay) i.e. for the [rubasd container](https://hub.docker.com/r/rubaspay/rubasd).
...
to run terminal commands from inside the terminal and build Rubas Core.
